...
const doctorApprove = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        if (caseData.status.status !== CaseStatus.FINISHED) {
            return next(new AppError('Can\'t approve this case now', 400));
        }

        const caseStatusDate = await caseStatus.findOne({
            where: {
                status: CaseStatus.DOCTORAPPROVED
            }
        });
        ...
        if (!caseStatusDate) {
            return next(new AppError('Finish status not found', 404));
        }
