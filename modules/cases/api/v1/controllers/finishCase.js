...
const finishCase = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        if (caseData.status.status !== CaseStatus.INREVIEW) {
            return next(new AppError('Can\'t finish this case now', 400));
        }
        ...
        res.status(200).json({
            status: 'success',
            message: 'Case Finished successfully',
        });
    } catch (error) {
        console.log(error);
